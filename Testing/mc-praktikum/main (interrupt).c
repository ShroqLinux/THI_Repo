...
void TIM7_IRQHandler(void) {
	...
	if ( (GPIOA->IDR & 1) != 0) {  // If button pressed

		onesec_cnt = 0; // reset count to 0
		onesec_cnt++;
		GPIOD -> ODR |= 1 << 13;		// activate pd13 for display

		LCD_ClearDisplay( 0xFE00 );
		LCD_WriteString( 10, 10, 0xFFFF, 0x0000, "Hallo Welt!");

		GPIOD -> ODR &= ~(1u << 12); // Pin green to 0
...
int main (void) {
    ...
    while ( 1 ) {
			...
			while (sys_ms <= (currSysCnt + 50)) { // wait to reach 50ms system time
